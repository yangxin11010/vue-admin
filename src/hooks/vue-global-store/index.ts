...
function getValue(value: symbol | string): string {
...
export class GlobalState<T extends object> {
    ...
    constructor(options: GlobalStateOptions<T>) {
        ...
        this.state = reactive(JSON.parse(stroage.getItem(getValue(name))) || (isObject(state) ? state : {}));

        watch(
            () => this.state,
            (value: T) => {
                stroage.setItem(getValue(name), JSON.stringify(value));
            },
            {
                deep: true,
            }
